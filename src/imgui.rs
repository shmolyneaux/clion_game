...
pub unsafe fn imgui_debug_inner(peek: &Peek, attributes: &[FieldAttribute], path: &str) {
    ...
    match (peek.scalar_type(), shape.def) {
        (Some(ScalarType::Unit), _) => igText(CString::new("()".to_string()).unwrap().as_ptr()),
        ...
        (Some(ScalarType::Str), _) => igText(
            CString::new(peek.get::<&str>().unwrap().to_string())
                .unwrap()
                .as_ptr(),
        ),
        (Some(ScalarType::String), _) => igText(
            CString::new(peek.get::<String>().unwrap().to_string())
                .unwrap()
                .as_ptr(),
        ),
        ...
        (_, Def::Scalar(def)) => igText(CString::new("Def::Scalar".to_string()).unwrap().as_ptr()),
        (_, Def::List(def)) => {
            let shape_typename = format_shape_typename(peek.shape());
            let peek = peek.into_list_like().unwrap();
            if peek.is_empty() {
                igText(CString::new("[]".to_string()).unwrap().as_ptr());
            } else if igTreeNode(
        ...
        (_, Def::Map(def)) => igText(CString::new("Def::Map".to_string()).unwrap().as_ptr()),
        (_, Def::Set(def)) => igText(CString::new("Def::Set".to_string()).unwrap().as_ptr()),
        (_, Def::Array(def)) => {
            let shape_typename = format_shape_typename(peek.shape());
            let peek = peek.into_list_like().unwrap();
            if peek.is_empty() {
        ...
        (_, Def::Slice(def)) => igText(CString::new("Def::Slice".to_string()).unwrap().as_ptr()),
        (_, Def::Option(def)) => igText(CString::new("Def::Option".to_string()).unwrap().as_ptr()),
        (_, Def::SmartPointer(def)) => {
            igText(CString::new("Def::SmartPointer".to_string()).unwrap().as_ptr())
        }
        (_, Def::Undefined) => {
            let ty = shape.ty;
            match ty {
                Type::Primitive(ty) => {
                    igText(CString::new("Type::Primitive".to_string()).unwrap().as_ptr())
                }
                Type::Sequence(ty) => {
                    igText(CString::new("Type::Sequence".to_string()).unwrap().as_ptr())
                }
                ...
                Type::User(UserType::Enum(ty)) => {
                    igText(CString::new("UserType::Enum".to_string()).unwrap().as_ptr())
                }
                Type::User(UserType::Union(ty)) => {
                    igText(CString::new("UserType::Union".to_string()).unwrap().as_ptr())
                }
                Type::User(UserType::Opaque) => {
                    igText(CString::new("UserType::Opaque".to_string()).unwrap().as_ptr())
                }
                Type::Pointer(ty) => {
                    igText(CString::new("Type::Pointer".to_string()).unwrap().as_ptr())
                }
                _ => igText(
                    CString::new(format!(
                        "Can't debug {}",
