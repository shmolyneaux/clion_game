...
pub fn parse_block_inner(tokens: &mut TokenStream) -> Result<Block, String> {
    ...
    while !tokens.is_empty() && *tokens.peek()? != Token::RCurly {
        ...
        let stmt = if *tokens.peek()? == Token::Let {
            ...
            let target = if *tokens.peek()? == Token::LBracket {
                tokens.advance()?;
                let mut idents = Vec::new();
                while !tokens.is_empty() && *tokens.peek()? != Token::RBracket {
                    match tokens.pop()? {
                        Token::Identifier(ident) => idents.push(ident),
                        token => {
                            tokens.unadvance()?;
                            return Err(tokens.format_peek_err(&format!(
                                "Expected ident in tuple pattern, found {:?}",
                                token
                            )));
                        }
                    }
                ...
                match tokens.pop()? {
                    Token::Identifier(ident) => Target::Ident(ident),
                    token => {
                        tokens.unadvance()?;
                        return Err(tokens
                            ...
                            for item in items {
                                match item.data {
                                    ...
                                    _ => return Err(format_script_err(
                                        item.span,
                                        &tokens.script,
                                        "Expected identifier in tuple assignment target",
                                    )),
                                }
                            }
                            let expr_to_assign = parse_expression(tokens)?;
